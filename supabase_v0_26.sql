-- EWP Material Forecast V0.26 — cumulative database migration
-- Run this once in Supabase SQL Editor BEFORE deploying the V0.26 frontend.
-- This file supersedes the older V0.17/V0.19/V0.20 SQL files and is intentionally idempotent.
-- It includes the existing inventory/purchasing/Spruce schema plus the V0.26 last-package and over-forecast import rules.
...
-- ---------------------------------------------------------------------------
-- V0.26 final-package workflow + over-forecast delivery imports
-- ---------------------------------------------------------------------------

begin;

-- A last-package flag closes future forecast for the entire level while keeping
-- the original Javelin takeoff quantities unchanged for audit/reference.
alter table public.spruce_orders
  add column if not exists is_last_package boolean not null default false;

-- Only one Spruce order can be the current last package for a level. When a new
-- order is marked last, clear the flag from any prior order on that level.
create or replace function public.keep_single_last_package()
returns trigger
language plpgsql
as $$
begin
  if coalesce(new.is_last_package, false) then
    update public.spruce_orders
    ...
    where level_id = new.level_id
      and id <> new.id
      and is_last_package = true;
  end if;
  return new;
end;
$$;

drop trigger if exists keep_single_last_package_trigger on public.spruce_orders;
create trigger keep_single_last_package_trigger
before insert or update of is_last_package, level_id
on public.spruce_orders
for each row execute function public.keep_single_last_package();

-- V0.26 intentionally allows a Spruce quantity to exceed the current forecast.
-- The delivery PDF is treated as the real package quantity; the UI warns users
-- about the overage, but the database no longer blocks it. Keep only the
-- structural integrity check that the material belongs to the same level.
create or replace function public.validate_spruce_order_item_quantity()
returns trigger
language plpgsql
as $$
declare
  v_order_level uuid;
  v_material_level uuid;
begin
  if coalesce(new.quantity_lf, 0) <= 0 then
    raise exception 'Spruce quantity must be greater than zero.';
  end if;

  select level_id into v_order_level
  from public.spruce_orders
  where id = new.spruce_order_id;

  select level_id into v_material_level
  from public.materials
  where id = new.material_id;

  if v_order_level is null or v_material_level is null then
    raise exception 'Spruce order or material no longer exists.';
  end if;

  if v_order_level <> v_material_level then
    raise exception 'Spruce material does not belong to the same level as the order.';
  end if;
...
-- Required/takeoff LF remains the original plan quantity. Actual delivered or
-- Spruce-committed LF may legitimately exceed it. The only remaining guard is
-- that a manual forecast exclusion cannot itself exceed the takeoff quantity.
create or replace function public.validate_material_required_lf()
returns trigger
language plpgsql
as $$
begin
  if coalesce(new.original_lf, 0) < 0 then
    raise exception 'Required LF cannot be negative for material %.', new.id;
  end if;
  if coalesce(new.excluded_lf, 0) < 0 then
    raise exception 'Excluded LF cannot be negative for material %.', new.id;
  end if;
  if coalesce(new.excluded_lf, 0) > coalesce(new.original_lf, 0) + 0.0001 then
    raise exception 'Excluded LF cannot exceed original forecast LF for material %.', new.id;
  end if;
  return new;
end;
$$;

-- Replace the V0.20 import RPC with the V0.26 signature. The extra flag is
-- stored on the order so removing/revising that order automatically restores or
-- re-closes the level forecast without rewriting the original takeoff rows.
drop function if exists public.upsert_spruce_order_import(uuid, text, text, text, text, text, text, jsonb);
...
create or replace function public.upsert_spruce_order_import(
  ...
  p_source_revision text default null,
  p_source_level_name text default null,
  p_is_last_package boolean default false,
  p_items jsonb default '[]'::jsonb
)
returns uuid
language plpgsql
...
begin
  ...
  -- Serialize delivery-name and last-package changes for the same level.
  perform 1 from public.levels where id = p_level_id for update;
  if not found then
    raise exception 'Level % no longer exists.', p_level_id;
  end if;
  ...
  if v_order_id is null then
    ...
    insert into public.spruce_orders (
      ...
      nullif(btrim(coalesce(p_source_revision, '')), ''),
      nullif(btrim(coalesce(p_source_level_name, '')), ''),
      coalesce(p_is_last_package, false), 1
    );
  else
    update public.spruce_orders
    set delivery_code = v_code,
        ...
        source_revision = nullif(btrim(coalesce(p_source_revision, '')), ''),
        source_level_name = nullif(btrim(coalesce(p_source_level_name, '')), ''),
        is_last_package = coalesce(p_is_last_package, false),
        updated_at = now(),
        version = version + 1
    where id = v_order_id;
  ...
  -- Reject material IDs that do not belong to this active level before insert.
  -- Quantities may exceed the original forecast in V0.26; that is a warning in
  -- the UI, not a database error.
  if exists (
    select 1
    from jsonb_to_recordset(p_items) as x(material_id uuid, material_name text, quantity_lf numeric)
    left join public.materials m on m.id = x.material_id
...
grant execute on function public.upsert_spruce_order_import(uuid, text, text, text, text, text, text, boolean, jsonb) to authenticated;
